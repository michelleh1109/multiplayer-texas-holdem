(** Test Plan: Most of our tests utilize glass-box testing in which we wrote
    ...
    test the player module more quicklmy and effectively than individual OUnit
    tests would allow. *)
...
(** [find_winner_test name p1 p2 cards expected_output] constructs an OUnit test
    named [name] that compares [expected_output] with [find_winner p1 p2 cards] *)
let find_winner_test (name : string) (p1 : Player.t) (p2 : Player.t)
    (cards : Card.t list) (expected_output : string) : test =
  name >:: fun _ ->
  assert_equal expected_output
    (Player.get_name (Compare.find_winner p1 p2 cards))
    ~printer:(fun n -> n)

let compare_tests =
  [
    find_winner_test "testing player with pair and other with high card 1"
      (Player.make_player_hole
         (Card.create_card_list [ (1, 'S'); (1, 'C') ])
         "p1")
      (Player.make_player_hole
         (Card.create_card_list [ (13, 'S'); (12, 'S') ])
         "p2")
      (Card.create_card_list
         [ (5, 'C'); (4, 'H'); (11, 'S'); (3, 'S'); (7, 'C') ])
      "p1";
    find_winner_test "testing player with pair and other with straight"
      (Player.make_player_hole
         (Card.create_card_list [ (2, 'S'); (3, 'C') ])
         "p1")
      (Player.make_player_hole
         (Card.create_card_list [ (4, 'H'); (5, 'D') ])
         "p2")
      (Card.create_card_list
         [ (2, 'D'); (6, 'H'); (7, 'S'); (8, 'C'); (9, 'D') ])
      "p2";
    find_winner_test
      "testing player with two pair (higher rank) and other with two pair \
       (lower rank)"
      (Player.make_player_hole
         (Card.create_card_list [ (10, 'S'); (10, 'C') ])
         "p1")
      (Player.make_player_hole
         (Card.create_card_list [ (5, 'H'); (5, 'D') ])
         "p2")
      (Card.create_card_list
         [ (6, 'D'); (6, 'H'); (9, 'S'); (9, 'C'); (11, 'D') ])
      "p1";
    find_winner_test
      "testing player with straight and other with three of a kind"
      (Player.make_player_hole
         (Card.create_card_list [ (1, 'S'); (2, 'C') ])
         "p1")
      (Player.make_player_hole
         (Card.create_card_list [ (3, 'S'); (3, 'C') ])
         "p2")
      (Card.create_card_list
         [ (3, 'D'); (4, 'S'); (5, 'C'); (6, 'S'); (7, 'D') ])
      "p1";
    find_winner_test
      "testing player with low straight and other with high straight"
      (Player.make_player_hole
         (Card.create_card_list [ (1, 'S'); (2, 'C') ])
         "p1")
      (Player.make_player_hole
         (Card.create_card_list [ (8, 'S'); (9, 'C') ])
         "p2")
      (Card.create_card_list
         [ (3, 'D'); (4, 'S'); (5, 'C'); (6, 'S'); (7, 'D') ])
      "p2";
    find_winner_test
      ...
      "p1";
    find_winner_test
      "testing player with low straight flush and other with high straight \
       flush 2"
      (Player.make_player_hole
         (Card.create_card_list [ (1, 'S'); (2, 'S') ])
         "p1")
      (Player.make_player_hole
         (Card.create_card_list [ (8, 'S'); (9, 'S') ])
         "p2")
      (Card.create_card_list
         [ (3, 'S'); (4, 'S'); (5, 'S'); (6, 'S'); (7, 'S') ])
      "p2";
    find_winner_test "testing player with low pair and other with high pair"
      (Player.make_player_hole
         (Card.create_card_list [ (1, 'S'); (2, 'C') ])
         "p1")
      (Player.make_player_hole
         (Card.create_card_list [ (10, 'S'); (11, 'C') ])
         "p2")
      (Card.create_card_list
         [ (3, 'D'); (3, 'S'); (5, 'C'); (6, 'S'); (7, 'D') ])
      "p2";
    find_winner_test "testing player with low pair and other with high pair 2"
      (Player.make_player_hole
         (Card.create_card_list [ (10, 'S'); (11, 'C') ])
         "p1")
      (Player.make_player_hole
      ...
      "p1m";
    find_winner_test
      "testing player with low three of a kind and other with high three of a \
       kind"
      (Player.make_player_hole
         (Card.create_card_list [ (1, 'S'); (1, 'C') ])
         "p1")
      (Player.make_player_hole
         ...
         "p2")
      (Card.create_card_list
         [ (1, 'D'); (3, 'S'); (5, 'C'); (6, 'S'); (10, 'D') ])
      "p2";
    find_winner_test
      "testing player with low three of a kind and other with high three of a \
       kind 2"
      (Player.make_player_hole
         (Card.create_card_list [ (7, 'S'); (10, 'C') ])
         "p1")
      (Player.make_player_hole
         (Card.create_card_list [ (5, 'S'); (1, 'C') ])
         "p2")
      (Card.create_card_list
         [ (1, 'D'); (1, 'S'); (4, 'C'); (10, 'S'); (10, 'D') ])
      "p2";
    find_winner_test
      "testing player with low four of a kind and other with high four of a \
       kind"
      (Player.make_player_hole
         (Card.create_card_list [ (1, 'S'); (1, 'C') ])
         "p1")
    ...
    find_winner_test
      ...
      "p1";
    find_winner_test
      "testing player with low full house and other with high full house"
      (Player.make_player_hole
         (Card.create_card_list [ (5, 'S'); (7, 'C') ])
         "p1")
      (Player.make_player_hole
         (Card.create_card_list [ (1, 'S'); (8, 'C') ])
         "p2")
      (Card.create_card_list
         [ (1, 'D'); (1, 'H'); (5, 'C'); (5, 'H'); (10, 'D') ])
      "p1";
    find_winner_test "testing player both with the same full house "
      (Player.make_player_hole
         (Card.create_card_list [ (5, 'S'); (7, 'C') ])
         "p1")
      (Player.make_player_hole
    ...
    find_winner_test
      ...
      (Card.create_card_list
         [ (1, 'D'); (1, 'H'); (5, 'C'); (5, 'H'); (10, 'D') ])
      "p1";
    find_winner_test "testing player with straight flush and other with pair"
      (Player.make_player_hole
         (Card.create_card_list [ (1, 'S'); (3, 'S') ])
         "p1")
      (Player.make_player_hole
         (Card.create_card_list [ (1, 'C'); (2, 'D') ])
         "p2")
      (Card.create_card_list
         [ (2, 'S'); (4, 'S'); (5, 'S'); (6, 'D'); (7, 'C') ])
      "p1";
    find_winner_test
      "testing player with full house and other with four of a kind"
      (Player.make_player_hole
         (Card.create_card_list [ (2, 'S'); (2, 'C') ])
         "p1")
      (Player.make_player_hole
         (Card.create_card_list [ (1, 'H'); (3, 'H') ])
         "p2")
      (Card.create_card_list
         [ (2, 'D'); (3, 'S'); (3, 'C'); (3, 'D'); (5, 'H'); (6, 'H') ])
      "p2";
    find_winner_test "testing players both with flush"
      (Player.make_player_hole
         (Card.create_card_list [ (10, 'S'); (9, 'D') ])
         "p1")
      (Player.make_player_hole
         (Card.create_card_list [ (2, 'C'); (2, 'H') ])
         "p2")
      (Card.create_card_list
         [ (4, 'S'); (5, 'C'); (6, 'D'); (7, 'H'); (8, 'S') ])
      "p1";
    find_winner_test "testing player with straight and other with two pairs"
      (Player.make_player_hole
         (Card.create_card_list [ (4, 'S'); (5, 'C') ])
         "p1")
      (Player.make_player_hole
         (Card.create_card_list [ (2, 'H'); (2, 'D') ])
         "p2")
      (Card.create_card_list
         [ (6, 'S'); (7, 'C'); (8, 'D'); (3, 'H'); (3, 'S') ])
      "p1";
    find_winner_test "testing player with flush and other with three of a kind"
      (Player.make_player_hole
         (Card.create_card_list [ (2, 'C'); (2, 'H') ])
         "p1")
      (Player.make_player_hole
